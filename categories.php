...
<div class="fluid-container">
  ...
  <?php
  if (isset($error)) {
    echo "<div class='alet alert-danger' style='text-align:center'>No Category Found!</div>";
    exit;
  }
  ...
  $post_per_page = 3;
  $status = "Published";
  $query1 = "SELECT * FROM post WHERE post_cat_id = :id AND post_status = :status";
  ...
  <section id="main" class="mx-5">
    ...
    <?php
    $status ='Published';
    $query2 = "SELECT * FROM post WHERE post_cat_id = :post_cat_id AND post_status = :status LIMIT $page_id, $post_per_page";
    ...
    if ($count1 == 0) {
      echo "<div class='alert alert-danger' style='text-align:center'>No Post Available !</div>";
    }else {
  ...
  if ($post_count > $post_per_page) { ?>
    ...
    <ul class="pagination px-5 justify-content-center">
      ...
      $page_id = 0;
      if ($first  != $page_id) {
        echo '<li class="page-item"><a class="page-link" href="categories.php?id=' . $_GET['id'] . '&page=' . $first . '" tabindex="-1">First</a></li>';
      } else {
        echo '<li class="page-item disabled"><a class="page-link" href="#" tabindex="-1">First</a></li>';
      }
      ?>
      <?php
      if (isset($_GET['page'])) {
      ...
      for ($i = 1; $i <= $total_page; $i++) {
        if ($i == $active) {
          echo '<li class="page-item active"><a class="page-link" href="categories.php?key=' . $_GET['id'] . '&page=' . $i . '">' . $i . '</a></li>';
        } else {
      ...
      if ($last  <= $total_page) {
        ...
        <a class="page-link" href="categories.php?id=' . $_GET['id'] . '&page=' . $last . '">Last</a>
        </li>';
      } else {
